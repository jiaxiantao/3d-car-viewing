...
```mermaid
flowchart TB
  subgraph ui [Next.js UI 层]
    Page["page.tsx\n状态 / 交互面板"]
    URL["use-showroom-url-state\n深链同步"]
  end
  subgraph r3f [React Three Fiber 场景]
  ...
  subgraph lib [核心库]
    ...
    Profile["market-rig-profiles.ts\n车型覆盖规则"]
...
> 以下截图请自行替换。建议将实际截图放在 `documentation/images/` 目录，并更新下方路径。

### 2.1 主界面 — 整车 WebGL 展示

![主界面全景 — 待替换](./images/placeholder-01-main-view.png)

> **📸 待截图说明**：打开首页默认 SUV 车型，展示 3D 画布、顶部快捷工具栏（场景模式 / 截图 / 全屏）、底部车型切换与交互 Tab。建议 16:9 宽屏，画布占画面主体。

### 2.2 车身交互 — 车门 / 灯光 / 车漆

![车身交互面板 — 待替换](./images/placeholder-02-interaction.png)

> **📸 待截图说明**：展开「车身交互」Tab，展示车门、后备箱、天窗、车灯、双闪等按钮；若有部件被禁用，一并截出禁用态与提示文案。

### 2.3 驾驶动态 — 启动 / 制动 / 环车巡检

![驾驶动态 — 待替换](./images/placeholder-03-drive.png)

> **📸 待截图说明**：切换到「驾驶动态」Tab，启动发动机并开启车灯；可配合制动（尾灯亮起）或自动环车巡检（`T` 键）截图。

### 2.4 多机位视角

![多机位预设 — 待替换](./images/placeholder-04-camera-presets.png)

> **📸 待截图说明**：依次或拼图展示「全景 / 前脸 / 左侧 / 右侧 / 车尾 / 驾舱」六个预设机位，体现 `CameraRig` 平滑过渡效果。

### 2.5 场景模式切换

![影棚 / 白天 / 夜晚 — 待替换](./images/placeholder-05-scene-modes.png)

> **📸 待截图说明**：同一车型下切换「影棚 / 白天 / 夜晚」三种模式，突出夜晚湿地反射与大灯光束。

### 2.6 深链分享

![URL 深链 — 待替换](./images/placeholder-06-deep-link.png)

> **📸 待截图说明**：地址栏展示形如 `?model=suv&paint=midnight&camera=front&mode=night` 的查询参数，刷新后状态保持一致。

### 2.7 开发调试 — GLB 部件识别面板

![GLB 部件识别 — 待替换](./images/placeholder-07-debug-panel.png)

> **📸 待截图说明**（仅开发环境可见）：展开 Debug 面板，展示 `discoverAssetCarRig` 对各部件的识别结果（✓ / —）。
...
![性能相关配置 — 待替换](./images/placeholder-08-performance.png)

> **📸 待截图说明**（可选）：Chrome DevTools Performance 或 FPS 监控，对比开启 `AdaptiveDpr` 前后的帧率（移动端更有意义）。
...
详细命名约定见 [market-glb-rig.md](./market-glb-rig.md)。
...
| `documentation/ARCHITECTURE.md` | 贡献者向架构说明 |
| `documentation/market-glb-rig.md` | GLB 建模与命名要求 |
...
| 项 | 链接 |
|----|------|
| GitHub 仓库 | [https://github.com/jiaxiantao/3d-car-viewing](https://github.com/jiaxiantao/3d-car-viewing) |
| Clone 地址 | `https://github.com/jiaxiantao/3d-car-viewing.git` |
| Issue / 讨论 | [https://github.com/jiaxiantao/3d-car-viewing/issues](https://github.com/jiaxiantao/3d-car-viewing/issues) |
...
| 主题 | 标题 | 链接 |
|------|------|------|
| Three.js 官方手册 | Three.js Manual | [https://threejs.org/manual/](https://threejs.org/manual/) |
| React Three Fiber | R3F 文档（Getting Started / API） | [https://docs.pmnd.rs/react-three-fiber](https://docs.pmnd.rs/react-three-fiber) |
| drei 辅助库 | @react-three/drei 文档与示例 | [https://github.com/pmndrs/drei](https://github.com/pmndrs/drei) |
| glTF 规范 | Khronos glTF 2.0 Specification | [https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html](https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html) |
| GLB 调试 | gltf.report — 在线查看 mesh / 材质结构 | [https://gltf.report/](https://gltf.report/) |
| 环境光照 | Three.js RoomEnvironment 示例（PMREM / IBL） | [https://threejs.org/docs/#examples/en/environments/RoomEnvironment](https://threejs.org/docs/#examples/en/environments/RoomEnvironment) |
| Next.js | Next.js App Router 文档 | [https://nextjs.org/docs/app](https://nextjs.org/docs/app) |
| 性能 | R3F Performance pitfalls | [https://docs.pmnd.rs/react-three-fiber/advanced/pitfalls](https://docs.pmnd.rs/react-three-fiber/advanced/pitfalls) |
...
*文档版本：与仓库 main 分支同步编写。截图占位路径：`documentation/images/placeholder-*.png`，请按需替换为实际截图。*
